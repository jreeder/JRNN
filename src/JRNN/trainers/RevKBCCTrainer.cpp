/********************************************************************
	created:	2013/05/29
	created:	29:5:2013   2:24
	filename: 	RevKBCCTrainer.cpp
	author:		jreeder
	
	purpose:	RevKBCC Trainer
*********************************************************************/
...
namespace JRNN {
	RandomGenerator01 RevKBCCTrainer::revRand = RandomGenerator01();

	RevKBCCTrainer::RevKBCCTrainer( int numIn, int numOut, int numCandidates, string outNodeType /*= ASigmoid::_type*/, string autoAssocType /*= ASigmoid::_type*/ )
	{
		net1 = RevKBCCNetwork::Create();
		net2 = RevKBCCNetwork::Create();
	...
	vecDouble RevKBCCTrainer::ActivateNet( vecDouble inPoint, vecDouble outPoint )
	{
	...
	vecDouble RevKBCCTrainer::Activate( vecDouble inPoint )
	{
	...
	double RevKBCCTrainer::TestOnData(DatasetPtr testData, Dataset::datatype type)
	{
		double retVal;
	...
	//double RevKBCCTrainer::TestOnData( Dataset::datatype type, DatasetPtr testData /*= DatasetPtr()*/, bool useTrueOuts /*= true*/ )
	//{
	//	double retVal;
	//	network = net1;
	//	DatasetPtr oldData = this->data;
	...
	JRNN::hashedDoubleMap RevKBCCTrainer::TestWiClass(DatasetPtr testData, Dataset::datatype type)
	{
	...
	void RevKBCCTrainer::TrainTask( DatasetPtr taskData,int maxEpochs, bool validate, bool testWhileTrain /*= false*/, DatasetPtr testData /*= DatasetPtr()*/, Dataset::datatype testDataType /*= Dataset::TRAIN*/ )
	{
	...
	void RevKBCCTrainer::FillBufferDS( int numPoints, bool validate /*= false */ )
	{
		matDouble newInputs;
	...
	CCTrainer::status RevKBCCTrainer::TrainOuts()
	{
		int quitEpoch = 0;
	...
	const RevKBCCTrainer::TestResults& RevKBCCTrainer::getTestWhileTrainResults()
	{
		return TestWhileTrainResults;
	}

	void RevKBCCTrainer::SaveNetParameters( netparameters& netparms )
	{
	...
	void RevKBCCTrainer::SetScaleAndOffset( double scale, double offset )
	{
		net1->SetScaleAndOffset(scale, offset);
		net2->SetScaleAndOffset(scale, offset);
	}

	void RevKBCCTrainer::SetSDCCandVaryActFunc( bool useSDCC, bool varyActFunc )
	{
	...
	//void RevKBCCTrainer::AddNewInputs( ints inputIndexes, DatasetPtr newData, bool connectToHidden /*= false */ )
	//{
	//	network = net1;
	//	NodeList addedNodes;
	...
	//void RevKBCCTrainer::AddInputsToNet( ints inputIndexes, RevKBCCNetworkPtr inNet, NodeList &addedNodes )
	//{
	...
	void RevKBCCTrainer::AddPrevTrainedNets( RevKBCCNetworkPtr inNet1, RevKBCCNetworkPtr inNet2 )
	{
		net1 = inNet1;
		net2 = inNet2;
		firstTrained = true;
	...
	JRNN::RevKBCCNetworkPtr RevKBCCTrainer::GetNet1()
	{
		return net1;
	}

	JRNN::RevKBCCNetworkPtr RevKBCCTrainer::GetNet2()
	{
		return net2;
	}
